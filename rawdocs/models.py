...
class MetadataLog(models.Model):
    """Journal des modifications des métadonnées"""
...
class DocumentPage(models.Model):
    ...
    # Validation humaine
    is_validated_by_human = models.BooleanField(default=False, help_text="Page validée par un humain")
...
class AnnotationType(models.Model):
    ...
    # Constantes pour les types prédéfinis
...
class Annotation(models.Model):
    ...
    # Score et raisonnement IA
    confidence_score = models.FloatField(default=0.0, help_text="Score de confiance (0.0–1.0)")
...
class AnnotationFeedback(models.Model):
    ...
    def __str__(self):
        return f"Feedback {self.pk} - Page {self.page.page_number}"

class AILearningMetrics(models.Model):
    """Suivi des performances de l'IA au fil du temps."""
    ...
    def __str__(self):
        return f"Metrics {self.created_at.date()} - F1: {self.f1_score:.2f}"

class PromptOptimization(models.Model):
    """Optimisation des prompts basée sur l'apprentissage."""
    ...
    def __str__(self):
        return f"Optimized prompt for {self.entity_type} (score: {self.performance_score:.2f})"