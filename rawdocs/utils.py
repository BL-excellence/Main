...
def extract_full_text(file_path):
    """
    Extraction de texte sans dépendre d'UltraAdvanced.
    Utilise un fallback robuste via pdfplumber pour obtenir un texte lisible et structuré.
    """
    try:
        return _extract_full_text_fallback(file_path)
    except Exception as e:
        print(f"⚠️ Error extracting text with fallback: {e}")
        return ""
